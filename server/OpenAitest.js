...
async function fetchTriviaQuestion() {
  const promptMessage = 'Generate a question with 4 possible answers, indicating the correct one with an asterisk. Be creative';
  ...
  if (response) {
    ...
    rl.question('Your answer (number): ', async (userAnswer) => {
      ...
      if (!isNaN(userAnswerIndex) && userAnswerIndex >= 1 && userAnswerIndex <= parsedQuestion.answers.length) {
        result.isCorrect = parsedQuestion.answers[userAnswerIndex - 1] === parsedQuestion.correctAnswer;
        console.log(result.isCorrect ? 'Correct!' : `Wrong! The correct answer was: ${parsedQuestion.correctAnswer}`);
      } else {
